(() => {
	...
	async function renderAnalysis(subject) {
		...
		try {
			...
			const actualDifficultyText = [1,2,3,4,5].map((n) => `${n}:${actualDifficultyCounts[n] || 0}`).join(' / ');
			const mockDifficultyText = [1,2,3,4,5].map((n) => `${n}:${mockDifficultyCounts[n] || 0}`).join(' / ');
			const difficultyDistributionText = t(`난이도 분포(1~5) · 실제기출: ${actualDifficultyText} · 모의고사 전체: ${mockDifficultyText}`, `難易度分布(1〜5)・過去問: ${actualDifficultyText}・模擬試験全体: ${mockDifficultyText}`);
