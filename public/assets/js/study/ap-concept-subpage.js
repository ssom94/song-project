(() => {
  ...
  function questionsHtml(problemTypes, openAnswers) {
    ...
    return problemTypes.map((type) => `<section class="ap-problem-type-section">
      ...
      ${type.questions.map((q)=>`<article class="ap-question-card">
        <div class="ap-question-toolbar"><div class="ap-question-number">Q${q.no}</div>${Number.isSafeInteger(Number(q.id)) ? memoButton('question', q.id, `Q${q.no}`, 'ap-note-trigger-question') : ''}</div>
  ...
  function renderDetail(payload) {
    ...
    const back = document.getElementById('ap-detail-back'); if (back) back.href = `${prefix}/?part=${encodeURIComponent(c.examPart)}`;
    if (!root) return;
    root.innerHTML = `<div class="ap-detail-head"><div><p class="ap-eyebrow">CONCEPT DETAIL</p>
      ...
      <div class="ap-detail-tools">${memoButton('concept', null, c.code, 'ap-note-trigger-concept')}<button id="ap-detail-ko-toggle" class="ap-concept-lang-button" type="button" aria-pressed="false">한국어 같이 보기</button></div></div>
    ...
    window.dispatchEvent(new CustomEvent('ap:concept-rendered', { detail: { code: c.code } }));
  }
  function renderProblems(payload) {
    ...
    root.innerHTML = `<div class="ap-detail-head"><div><p class="ap-eyebrow">EXPECTED QUESTIONS</p><h1>${esc(c.code)} · ${esc(c.titleJa)} · 予想問題</h1>
      ...
      <div class="ap-ko-secondary" hidden><strong>${esc(c.titleKo)}</strong> · ${esc(c.unitKo)}</div></div>
      <div class="ap-detail-tools">${memoButton('concept', null, c.code, 'ap-note-trigger-concept')}<button id="ap-problem-ko-toggle" class="ap-concept-lang-button" type="button" aria-pressed="false">한국어 같이 보기</button></div></div>
