(() => {
  ...
  function normalizeAttempt(attempt, storageIndex) {
    ...
    return { storageIndex, at:attempt?.at, selected, scores, total, focus, source, focusMatched, durationSecondsUsed, timedOut:Boolean(attempt?.timedOut), mode:attempt?.mode || null };
  }
  ...
  function scoreDetail(attempt) { return attempt.selected.map((no)=>`Q${no} ${fieldLabel(no)} ${attempt.scores[no]==null?'-':`${attempt.scores[no]}/20`}`).join(' · '); }
  function locate() {
  ...
  function render() {
    ...
    el.innerHTML=`<div class="ap-mock-section-head"><div><p class="ap-mock-section-kicker">SUBJECT B HISTORY</p><h2>${esc(t('科目B 학습 이력','科目B 学習履歴'))}</h2></div><p>${esc(t(`최근 ${newest.length}회`,`直近${newest.length}回`))}</p></div><section class="ap-mock-card"><p class="ap-past-guide"><strong>${esc(t('최근 점수','直近スコア'))}:</strong> ${latest}/100 · <strong>${esc(t('평균','平均'))}:</strong> ${average.toFixed(1)}/100 · <strong>${esc(t('최고','最高'))}:</strong> ${best}/100</p><p class="ap-mock-note">${esc(t('최대 20회까지 브라우저에 보관합니다. 150분 실전은 소요시간과 시간만료 여부도 함께 기록합니다. 개별 삭제나 전체 초기화 후 선택전략·약점·점수추이·목표점수 판정도 다시 계산됩니다.','最大20回までブラウザに保存します。150分実戦は所要時間と時間切れかどうかも記録します。個別削除や全消去後は選択戦略・弱点・得点推移・目標得点判定も再計算されます。'))}</p><div class="ap-past-actions"><a class="ap-mock-button" href="/${lang}/study/ap/reinforcement/?subject=B">${esc(t('새 보정 실전','新しい補正実戦'))}</a><a class="ap-mock-button" href="/${lang}/study/ap/reinforcement/simulation/">${esc(t('150분 실전 시뮬레이션','150分実戦シミュレーション'))}</a><button class="ap-mock-button" type="button" data-b-history-clear>${esc(t('전체 이력 초기화','履歴をすべて消去'))}</button></div><div class="ap-mock-table-wrap"><table class="ap-mock-table"><thead><tr><th>${esc(t('회차','回'))}</th><th>${esc(t('실시일시','実施日時'))}</th><th>${esc(t('실시 방식','実施方式'))}</th><th>${esc(t('선택 분야·점수','選択分野・得点'))}</th><th>${esc(t('총점','合計'))}</th><th>${esc(t('소요시간','所要時間'))}</th><th>${esc(t('추천·실전 구분','推薦・実戦区分'))}</th><th>${esc(t('관리','管理'))}</th></tr></thead><tbody>${rows}</tbody></table></div><p class="ap-mock-note">${esc(t('답안 원문은 이 이력에 저장하지 않습니다. “기존 기록”은 추천 출처 메타데이터 저장 전의 이력입니다.','解答本文はこの履歴に保存しません。「既存記録」は推薦元メタデータ保存開始前の履歴です。'))}</p></section>`;
  }
  document.addEventListener('click',(event)=>{
    ...
    if (deleteButton) {
      ...
      if (!confirm(t('이 자기채점 기록 1건을 삭제할까요? 삭제하면 약점·추이·목표점수 판정도 다시 계산됩니다.','この自己採点履歴1件を削除しますか？削除すると弱点・推移・目標得点判定も再計算されます。'))) return;
