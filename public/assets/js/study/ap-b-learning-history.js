(() => {
  ...
  const fieldNames = {
    ...
    2: ['경영전략', '経営戦略'],
    3: ['프로그래밍', 'プログラミング'],
    4: ['시스템 아키텍처', 'システムアーキテクチャ'],
    5: ['네트워크', 'ネットワーク'],
    6: ['데이터베이스', 'データベース'],
    7: ['임베디드 시스템', '組込みシステム'],
    8: ['시스템 개발', 'システム開発'],
    9: ['프로젝트 관리', 'プロジェクトマネジメント'],
    10: ['서비스 관리', 'サービスマネジメント'],
    11: ['시스템 감사', 'システム監査'],
  };
  ...
  function readStore() {
    try {
      ...
      return { version: 1, attempts: Array.isArray(data?.attempts) ? data.attempts.slice(-20) : [] };
    } catch {
  ...
  function fieldLabel(no) {
    const pair = fieldNames[no] || [`Q${no}`, `Q${no}`];
    return t(pair[0], pair[1]);
  }
  function scoreDetail(attempt) {
    return attempt.selected.map((no) => {
      const score = attempt.scores[no];
      return `Q${no} ${fieldLabel(no)} ${score == null ? '-' : `${score}/20`}`;
    }).join(' · ');
  }
  function locate() {
    let el = document.getElementById('ap-b-learning-history');
    if (el) return el;
    ...
    const anchor = progress || weakness;
    if (!anchor?.parentElement) return null;
    el = document.createElement('section');
    el.id = 'ap-b-learning-history';
  ...
  function render() {
    ...
    if (!newest.length) {
      el.innerHTML = `<div class="ap-mock-section-head"><div><p class="ap-mock-section-kicker">SUBJECT B HISTORY</p><h2>${esc(t('科目B 학습 이력','科目B 学習履歴'))}</h2></div><p>0</p></div><section class="ap-mock-card"><p class="ap-past-guide">${esc(t('아직 저장된 자기채점 이력이 없습니다. 科目B 보정 실전을 풀고 자기채점을 저장하면 이곳에 회차별 기록이 쌓입니다.','保存済みの自己採点履歴はまだありません。科目B補正実戦を解いて自己採点を保存すると、ここに回ごとの履歴が蓄積されます。'))}</p><div class="ap-past-actions"><a class="ap-mock-button" href="/${lang}/study/ap/reinforcement/?subject=B">${esc(t('科目B 보정 실전 시작','科目B補正実戦を開始'))}</a></div></section>`;
      return;
    }

    const rows = newest.map((attempt) => {
      const round = attempt.storageIndex + 1;
      return `<tr><td><strong>#${round}</strong></td><td>${esc(formatDate(attempt.at))}</td><td>${esc(sourceLabel(attempt))}</td><td>${esc(scoreDetail(attempt))}</td><td><strong>${attempt.total}/100</strong></td><td>${esc(recommendationLabel(attempt))}</td><td><button class="ap-mock-button" type="button" data-b-history-delete="${attempt.storageIndex}">${esc(t('삭제','削除'))}</button></td></tr>`;
    }).join('');

    el.innerHTML = `<div class="ap-mock-section-head"><div><p class="ap-mock-section-kicker">SUBJECT B HISTORY</p><h2>${esc(t('科目B 학습 이력','科目B 学習履歴'))}</h2></div><p>${esc(t(`최근 ${newest.length}회`,`直近${newest.length}回`))}</p></div><section class="ap-mock-card"><p class="ap-past-guide"><strong>${esc(t('최근 점수','直近スコア'))}:</strong> ${latest}/100 · <strong>${esc(t('평균','平均'))}:</strong> ${average.toFixed(1)}/100 · <strong>${esc(t('최고','最高'))}:</strong> ${best}/100</p><p class="ap-mock-note">${esc(t('최대 20회까지 브라우저에 보관합니다. 개별 삭제나 전체 초기화 후에는 선택전략·약점·점수추이 판정도 즉시 다시 계산됩니다. 작성한 서술 답안 원문은 저장하지 않습니다.','最大20回までブラウザに保存します。個別削除または全消去後は、選択戦略・弱点・得点推移判定もすぐ再計算されます。記述した解答本文は保存しません。'))}</p><div class="ap-past-actions"><a class="ap-mock-button" href="/${lang}/study/ap/reinforcement/?subject=B">${esc(t('새 보정 실전 풀기','新しい補正実戦を解く'))}</a><button class="ap-mock-button" type="button" data-b-history-clear>${esc(t('전체 이력 초기화','履歴をすべて消去'))}</button></div><div class="ap-mock-table-wrap"><table class="ap-mock-table"><thead><tr><th>${esc(t('회차','回'))}</th><th>${esc(t('실시일시','実施日時'))}</th><th>${esc(t('실시 방식','実施方式'))}</th><th>${esc(t('선택 분야·점수','選択分野・得点'))}</th><th>${esc(t('총점','合計'))}</th><th>${esc(t('추천 보강 여부','推薦補強'))}</th><th>${esc(t('관리','管理'))}</th></tr></thead><tbody>${rows}</tbody></table></div><p class="ap-mock-note">${esc(t('“기존 기록”은 추천 출처 메타데이터를 저장하기 전에 생성된 이력이라 당시 자동추천 여부를 판정할 수 없습니다.','「既存記録」は推薦元メタデータの保存開始前に作成された履歴のため、当時の自動推薦利用有無を判定できません。'))}</p></section>`;
  }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (deleteButton) {
      ...
      if (!confirm(t('이 자기채점 기록 1건을 삭제할까요? 삭제하면 약점·추이 판정도 다시 계산됩니다.','この自己採点履歴1件を削除しますか？削除すると弱点・推移判定も再計算されます。'))) return;
      data.attempts.splice(index,1);
      writeStore(data.attempts);
      return;
    }
