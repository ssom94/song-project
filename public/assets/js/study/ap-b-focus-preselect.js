(() => {
  ...
  const source = ['weakness','progress'].includes(rawSource) ? rawSource : 'manual';
  ...
  function noticeText() {
    ...
    return t(`약점 대시보드 추천에 따라 Q${focus.join(', Q')}가 자동 선택되었습니다. 원하면 다른 분야로 바꿀 수 있습니다.`, `弱点ダッシュボードの推薦によりQ${focus.join('、Q')}を自動選択しました。必要なら別の分野へ変更できます。`);
  }
  function apply() {
