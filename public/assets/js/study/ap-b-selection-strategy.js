(() => {
  ...
  async function loadDifficulties() {
    const sets = await Promise.all(sessions.map((s)=>json(`/assets/data/ap-past-study/${s}-B.json`)));
    const out = {};
  ...
  function locate() {
    ...
    el = document.createElement('section'); el.id='ap-b-selection-strategy'; el.className='ap-mock-section';
    analysis.parentElement.insertBefore(el, analysis.nextSibling); return el;
  }
  function renderResult(el, difficulties, state) {
    ...
    const primary = ranked.slice(0,4), reserve = ranked.slice(4,6), avoid = ranked.slice(6);
  ...
  async function render() {
    ...
    el.querySelector('[data-strategy-calc]').addEventListener('click',()=>{ const next={}; el.querySelectorAll('[data-confidence]').forEach((s)=>next[s.dataset.confidence]=Number(s.value)); saveState(next); renderResult(el,difficulties,next); });
    el.querySelector('[data-strategy-reset]').addEventListener('click',()=>{ localStorage.removeItem(KEY); el.querySelectorAll('[data-confidence]').forEach((s)=>s.value='3'); el.querySelector('[data-strategy-result]').hidden=true; });
  }
  const observer = new MutationObserver(()=>{ if (document.getElementById('ap-mock-past-analysis-b')) { observer.disconnect(); render().catch((e)=>console.error('AP_B_SELECTION_STRATEGY_FAILED',e)); } });
