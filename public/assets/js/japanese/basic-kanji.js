(() => {
	...
	function renderRadicals() {
		ui.total.textContent=String(RADICALS.length); ui.mastered.textContent=text('핵심 '+RADICALS.length+'개','主要'+RADICALS.length+'個');
		const query=ui.search.value.trim().toLocaleLowerCase();
		const filtered=RADICALS.filter((row)=>!query||row.join(' ').toLocaleLowerCase().includes(query));
		ui.list.innerHTML=filtered.map((row)=>{
	...
	function render() {
		...
		ui.group.hidden=radical; ui.state.hidden=radical; ui.memoryStart.hidden=radical;
