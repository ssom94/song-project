(() => {
	...
	function copy() {
		return language() === 'ko'
			? {
				kicker: 'JLPT N1 · TODAY',
				title: '오늘의 학습',
				drag: '제목을 잡아 이동할 수 있습니다.',
				completed: (done, total) => `${done} / ${total} 스케줄 완료`,
				review: '복습 어휘',
				newWords: '신규 어휘',
				vocab: '어휘 문제',
				grammar: '문법',
				reading: '독해',
				wordHint: '단어 상태 지정 시 자동 완료',
				manualHint: '체크하여 완료 처리',
				publicHint: '관리자 로그인 시 완료 처리 가능',
				open: 'JLPT 학습 열기 →',
				startTomorrow: '학습 시작일 전입니다. 내일부터 Day 1을 시작합니다.',
				noPlan: '오늘 학습 정보를 불러오지 못했습니다.',
				reset: '위치 초기화',
				collapse: '접기',
				expand: '펼치기',
			}
			: {
				...
				grammar: '文法',
				reading: '読解',
				wordHint: '単語状態の設定で自動完了',
				manualHint: 'チェックで完了処理',
				publicHint: '管理者ログイン時に完了処理可能',
				open: 'JLPT学習を開く →',
				startTomorrow: '学習開始日前です。明日から Day 1 を開始します。',
				noPlan: '今日の学習情報を読み込めませんでした。',
				reset: '位置をリセット',
				collapse: '折りたたむ',
				expand: '展開する',
			};
	...
	function moveDrag(event) {
		...
		const position = clampPosition(event.clientX - dragging.offsetX, event.clientY - dragging.offsetY);
	...
	function buildWidget() {
		...
		widget.innerHTML = `
			<div class="jlpt-today-floating-head">
				<div class="jlpt-today-floating-title">
					<span data-jlpt-floating-kicker></span>
					<strong data-jlpt-floating-title></strong>
					<small data-jlpt-floating-drag></small>
				</div>
				<div class="jlpt-today-floating-actions">
					<button class="jlpt-today-floating-icon-button" type="button" data-jlpt-floating-reset>↺</button>
					<button class="jlpt-today-floating-icon-button" type="button" data-jlpt-floating-collapse>−</button>
				</div>
			</div>
		...
		widget.querySelector('[data-jlpt-floating-collapse]')?.addEventListener('click', () => {
	...
	function itemDone(item) {
		return item.target <= 0 || item.completed >= item.target;
	}
	...
	function render(data) {
		...
		const doneItems = activeItems.filter(itemDone).length;
		const totalItems = activeItems.length || 5;
		...
		setText('[data-jlpt-floating-summary]', labels.completed(doneItems, totalItems));
		setText('[data-jlpt-floating-percent]', `${percent}%`);
		setText('[data-jlpt-floating-date]', plan.today || '—');
		...
		if (reset instanceof HTMLButtonElement) {
			reset.title = labels.reset;
			reset.setAttribute('aria-label', labels.reset);
		}
		syncCollapsed();
		...
		for (const item of items) {
			...
			row.className = `jlpt-today-floating-item${done ? ' is-completed' : ''}`;
			const checkbox = document.createElement('input');
			checkbox.type = 'checkbox';
			checkbox.className = 'jlpt-today-floating-check';
			checkbox.checked = done;
			checkbox.dataset.section = item.key;
			const canManuallyUpdate = item.manual && authenticated && Boolean(data?.today?.sessionId) && item.target > 0;
			checkbox.disabled = !canManuallyUpdate;
			if (canManuallyUpdate) {
				checkbox.addEventListener('change', () => updateSection(item.key, checkbox.checked ? item.target : 0, checkbox));
			}
			...
			hint.textContent = item.manual
				? (authenticated ? labels.manualHint : labels.publicHint)
				: labels.wordHint;
	...
	function handleResize() {
		...
		if (!stored) return;
		const clamped = clampPosition(widget.getBoundingClientRect().left, widget.getBoundingClientRect().top);
		widget.style.left = `${clamped.x}px`;
		widget.style.top = `${clamped.y}px`;
		storePosition(clamped.x, clamped.y);
	}
