(() => {
	...
	function renderHistoricalSaveBar(card) {
		...
		if(!bar){bar=document.createElement('div');bar.className='jlpt-word-state-save-bar jlpt-history-save-bar';bar.innerHTML='<span></span><button type="button" class="jlpt-primary-button"></button>';bar.querySelector('button').addEventListener('click',()=>saveHistoricalStates());card.querySelector('.jlpt-card-heading')?.after(bar);}
		const count=archivePendingStates.size;bar.querySelector('span').textContent=count?t(`${count}개 단어의 상태가 변경되었습니다.`,`${count}語の状態が変更されました。`):t('변경할 상태를 선택한 뒤 한 번에 저장하세요.','状態を選択してからまとめて保存してください。');const button=bar.querySelector('button');button.disabled=count===0||archiveStatesSaving;button.textContent=archiveStatesSaving?t('저장 중…','保存中…'):(count?t(`상태 저장 (${count})`,`状態を保存 (${count})`):t('상태 저장','状態を保存'));
	}
