	...
	async function completeHistoricalWord(studyDate, word, state, actions) {
		...
		try {
			await requestJson(HISTORICAL_WORD_API, {
				...
				body: JSON.stringify({ studyDate, wordId: word.id, state }),
			});
			await selectDate(studyDate);
		} catch (error) {
			actions.querySelectorAll('button').forEach((button)=>button.disabled=false);
			const status = actions.previousElementSibling;
			if (status) status.textContent = error.status === 401 ? t('관리자 로그인 후 학습 상태를 저장할 수 있습니다.', '管理者ログイン後に学習状態を保存できます。') : t('학습 상태 저장에 실패했습니다.', '学習状態の保存に失敗しました。');
		}
	}
