(() => {
	...
	function calendarPreviewCard() {
		...
		card=document.createElement('section'); card.id='jlpt-calendar-preview-card'; card.className='jlpt-card jlpt-calendar-preview-card';
		const calendar=document.getElementById('jlpt-calendar')?.closest('.jlpt-card');
		(calendar?.parentElement||document.querySelector('.jlpt-content'))?.insertBefore(card,(calendar?.nextSibling)||null);
		return card;
	}
	function renderCalendarPreviewLoading(date) {
		...
		const heading=document.createElement('div'); heading.className='jlpt-card-heading';
		const copy=document.createElement('div'); const title=document.createElement('h2'); title.textContent=t(`${date} 학습 미리보기`,`${date} 学習プレビュー`);
		const note=document.createElement('p'); note.textContent=t('선택한 날짜의 단어를 불러오는 중입니다.','選択した日の単語を読み込んでいます。'); copy.append(title,note); heading.append(copy);
		card.append(heading, Object.assign(document.createElement('p'),{className:'jlpt-empty',textContent:t('불러오는 중입니다.','読み込んでいます。')}));
	}
	function renderCalendarPreview(data) {
		const card=calendarPreviewCard(); card.hidden=false; card.replaceChildren();
		const heading=document.createElement('div'); heading.className='jlpt-card-heading';
		const copy=document.createElement('div'); const title=document.createElement('h2'); title.textContent=t(`${data.studyDate} 학습 미리보기`,`${data.studyDate} 学習プレビュー`);
		const note=document.createElement('p'); note.textContent=t('달력에서 날짜를 선택했습니다. 아래에서 신규 단어와 준비된 문제 수를 바로 확인할 수 있습니다.','カレンダーで日付を選択しました。新規単語と準備済みの問題数をここで確認できます。'); copy.append(title,note);
		const open=document.createElement('button'); open.type='button'; open.className='jlpt-secondary-button'; open.textContent=t('전체 학습 내용 보기','学習内容を開く'); open.addEventListener('click',()=>selectDate(data.studyDate)); heading.append(copy,open); card.appendChild(heading);
		const words=Array.isArray(data.words)?data.words:[];
		const summary=document.createElement('div'); summary.className='jlpt-calendar-preview-summary';
		summary.textContent=t(`신규 단어 ${words.length}개 · 문제 ${(data.questions||[]).length}개 · 문법 ${(data.grammar||[]).length}개 · 독해 ${(data.readings||[]).length}개`,`新規単語 ${words.length}件 · 問題 ${(data.questions||[]).length}問 · 文法 ${(data.grammar||[]).length}件 · 読解 ${(data.readings||[]).length}件`); card.appendChild(summary);
		const list=document.createElement('div'); list.className='jlpt-calendar-preview-words';
		if(!words.length) list.appendChild(Object.assign(document.createElement('p'),{className:'jlpt-empty',textContent:t('이 날짜에 등록된 신규 단어가 없습니다.','この日に登録された新規単語はありません。')}));
		else words.forEach((word,index)=>{const item=document.createElement('article');item.className='jlpt-calendar-preview-word';const strong=document.createElement('strong');strong.textContent=`${index+1}. ${word.word||'—'}`;const reading=document.createElement('span');reading.textContent=word.reading||'—';const meaning=document.createElement('small');meaning.textContent=word.meaningKo||word.meaningJa||'—';item.append(strong,reading,meaning);list.appendChild(item);});
		card.appendChild(list);
	}
	async function previewCalendarDate(date) {
		if(!/^\d{4}-\d{2}-\d{2}$/.test(date)) return;
		calendarPreviewDate=date; updateDateLabel(); renderCalendarPreviewLoading(date);
		try { renderCalendarPreview(await requestJson(`${PRACTICE_API}?date=${encodeURIComponent(date)}`)); }
		catch(error) { const card=calendarPreviewCard(); card.replaceChildren(); card.hidden=false; card.appendChild(Object.assign(document.createElement('p'),{className:'jlpt-empty',textContent:error.status===400?t('조회할 수 없는 날짜입니다.','参照できない日付です。'):t('선택한 날짜의 미리보기를 불러오지 못했습니다.','選択した日のプレビューを読み込めませんでした。')})); }
	}
	...
	function setupCalendar(){ const cal=document.getElementById('jlpt-calendar');if(!cal)return;cal.addEventListener('click',(event)=>{const cell=event.target.closest('.jlpt-calendar-day');if(!cell)return;const date=cell.dataset.fullDate||calendarCellDate(cell);if(date)previewCalendarDate(date);});new MutationObserver(()=>updateDateLabel()).observe(cal,{childList:true}); }
