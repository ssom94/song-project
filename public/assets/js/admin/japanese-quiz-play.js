(() => {
	...
	function applyEnhancementCopy() {
		...
		if (panel) {
			...
			const labels = {
				mastered: copy('암기 완료', '習得済み'),
				uncertain: copy('애매함', 'あいまい'),
				unlearned: copy('미학습', '未習得'),
			};
			panel.querySelectorAll('[data-learning-state]').forEach((button) => {
				button.textContent = labels[button.dataset.learningState] || button.dataset.learningState;
			});
		}
	}
	...
	function renderQuestion() {
		...
		byId('quiz-play-type-badge').textContent = question.answerMode === 'choice' ? 'CHOICE' : question.type === 'reading' ? 'READING' : question.type === 'meaning' ? 'MEANING' : 'SENTENCE';
	...
	function lockChoices(selectedButton, isCorrect) {
		...
		byId('quiz-play-choices').querySelectorAll('button').forEach((button) => {
			button.disabled = true;
			if (window.JapaneseQuizEngine?.isCorrect?.(question, button.textContent)) button.classList.add('is-correct');
		});
	...
	function showFeedback(isCorrect, selectedButton = null, skipped = false) {
		...
		byId('quiz-play-feedback-note').textContent = copy(
	...
	async function saveResult() {
		...
		try {
			const payloadAttempts = attempts.map((attempt) => ({
				...
				learningState: attempt.learningState,
			}));
	...
	async function initialize() {
		...
		try {
			...
			await window.AdminCommon?.alert?.({
				...
				messageFallback: copy(
					'선택 조건의 단어 데이터를 확인해 주세요. 4지선다는 서로 다른 한국어 뜻이 최소 4개 필요하고, 예문 빈칸은 해당 단어가 포함된 등록 예문이 필요합니다.',
					'選択条件の単語データを確認してください。4択には異なる韓国語の意味が最低4つ必要で、例文穴埋めには対象単語を含む登録例文が必要です。',
				),
			});
