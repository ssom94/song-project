...
export async function handleGetPublicJapaneseJlptDashboard(request: Request, env: Env): Promise<Response> {
	try {
		...
		const [curriculum, registeredN1, stateCounts, reviewDue, session, calendarResult, contentResult] = await Promise.all([
			...
			// Keep genuine study history, plus dates prepared by the curated batch
			// pipeline. Legacy preview rows can have sessions, words, and generated
			// content but no stable wordKey, so they must not become calendar days.
			env.song_project_db.prepare(`SELECT s.study_date, s.status, s.review_completed, s.review_target, s.new_word_completed, s.new_word_target, s.vocab_question_completed, s.vocab_question_target, s.grammar_completed, s.grammar_target, s.reading_completed, s.reading_target FROM japanese_jlpt_daily_sessions AS s WHERE s.plan_id = ?1 AND (s.status <> 'not_started' OR s.review_completed + s.new_word_completed + s.vocab_question_completed + s.grammar_completed + s.reading_completed > 0 OR EXISTS (SELECT 1 FROM japanese_jlpt_daily_contents AS c WHERE c.plan_id = s.plan_id AND c.study_date = s.study_date AND c.content_type = 'vocab_question' AND json_extract(c.payload_json, '$.wordKey') IS NOT NULL LIMIT 1)) ORDER BY s.study_date DESC LIMIT 365`).bind(plan.id).all<CalendarRow>(),
