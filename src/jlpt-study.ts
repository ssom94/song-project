	...
	await db.batch([
		db.prepare(`
			CREATE TABLE IF NOT EXISTS japanese_jlpt_study_plans (
				...
				jlpt_level_code TEXT NOT NULL DEFAULT 'N1' CHECK (jlpt_level_code IN ('N1', 'N2', 'N3', 'N4', 'N5')),
				study_start_date TEXT NOT NULL,
				target_exam_date TEXT NOT NULL,
				target_date_is_tentative INTEGER NOT NULL DEFAULT 1 CHECK (target_date_is_tentative IN (0, 1)),
		...
		db.prepare(`
			CREATE TABLE IF NOT EXISTS japanese_jlpt_daily_contents (
				...
				plan_id INTEGER NOT NULL,
				study_date TEXT NOT NULL,
				content_type TEXT NOT NULL CHECK (content_type IN ('vocab_question', 'grammar', 'grammar_question', 'reading')),
...
export async function ensureDefaultJlptStudyPlan(db: D1Database, adminId: number): Promise<JlptStudyPlanRow> {
	...
	await db.prepare(`
		INSERT OR IGNORE INTO japanese_jlpt_study_plans
			(admin_id, plan_code, jlpt_level_code, study_start_date, target_exam_date, target_date_is_tentative,
			 target_word_count, daily_new_word_target, vocab_question_target, grammar_target, reading_target,
			 is_active, created_at, updated_at)
		VALUES (?1, ?2, 'N1', ?3, ?4, 1, ?5, ?6, ?7, ?8, ?9, 1, ?10, ?10)
	`).bind(
		adminId,
		DEFAULT_JLPT_PLAN_CODE,
		DEFAULT_JLPT_STUDY_START_DATE,
		DEFAULT_JLPT_TARGET_EXAM_DATE,
		DEFAULT_JLPT_TARGET_WORD_COUNT,
		DEFAULT_JLPT_DAILY_NEW_WORDS,
		DEFAULT_JLPT_VOCAB_QUESTIONS,
		DEFAULT_JLPT_GRAMMAR_TARGET,
		DEFAULT_JLPT_READING_TARGET,
		now,
	).run();

	const plan = await db.prepare(`
		SELECT id, admin_id, plan_code, jlpt_level_code, study_start_date, target_exam_date,
			target_date_is_tentative, target_word_count, daily_new_word_target, vocab_question_target,
			grammar_target, reading_target, is_active
		FROM japanese_jlpt_study_plans
		WHERE admin_id = ?1 AND plan_code = ?2
		LIMIT 1
	`).bind(adminId, DEFAULT_JLPT_PLAN_CODE).first<JlptStudyPlanRow>();
	if (plan) return plan;
	...
	`).bind(adminId).first<JlptStudyPlanRow>();
...
export function addDays(dateText: string, days: number): string {
	const base = Date.parse(`${dateText}T00:00:00+09:00`);
	if (Number.isNaN(base)) return dateText;
	return japanDateString(new Date(base + days * 86_400_000));
}
...
export function validDateText(value: unknown): string | null {
...
export async function enrollWordsInJlptPlan(
	...
	const uniqueIds = [...new Set(wordIds.filter((id) => Number.isSafeInteger(id) && id > 0))];
	if (!uniqueIds.length) return { added: 0, already: 0 };
	const maxRow = await db.prepare(`SELECT COALESCE(MAX(sort_order), 0) AS max_order FROM japanese_jlpt_curriculum_words WHERE plan_id = ?1`)
		.bind(planId).first<{ max_order: number }>();
	let nextOrder = Number(maxRow?.max_order ?? 0) + 1;
	let added = 0;
	let already = 0;
	for (const wordId of uniqueIds) {
		const existing = await db.prepare(`SELECT id FROM japanese_jlpt_curriculum_words WHERE plan_id = ?1 AND word_id = ?2 LIMIT 1`)
			.bind(planId, wordId).first<{ id: number }>();
		if (existing) {
			already += 1;
			await db.prepare(`
				UPDATE japanese_jlpt_curriculum_words
				SET introduced_on = COALESCE(introduced_on, ?3)
				WHERE plan_id = ?1 AND word_id = ?2
			`).bind(planId, wordId, introducedOn).run();
			continue;
		}
		await db.prepare(`
			INSERT INTO japanese_jlpt_curriculum_words (plan_id, word_id, sort_order, introduced_on)
			VALUES (?1, ?2, ?3, ?4)
		`).bind(planId, wordId, nextOrder, introducedOn).run();
		nextOrder += 1;
		added += 1;
	}
