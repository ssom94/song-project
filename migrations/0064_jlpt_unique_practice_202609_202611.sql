...
-- Rebuild 2026-09-07..2026-11-30 grammar questions and reading sets.
-- D1-safe version: no non-existent daily_words id column and no correlated OFFSET.
-- Existing word progress, vocabulary questions and grammar lessons are preserved.
...
-- Three date-specific grammar questions per day.
WITH RECURSIVE days(d,n) AS (
  ...
  SELECT d.d,d.n,p.id AS plan_id,
    COALESCE((SELECT g.pattern FROM grammar_pool g WHERE g.plan_id=p.id AND g.study_date=d.d AND g.sequence_no=1 LIMIT 1),'〜に即して') AS p1,
    COALESCE((SELECT g.meaning_ko FROM grammar_pool g WHERE g.plan_id=p.id AND g.study_date=d.d AND g.sequence_no=1 LIMIT 1),'〜에 입각하여') AS m1,
    COALESCE((SELECT g.pattern FROM grammar_pool g WHERE g.plan_id=p.id AND g.study_date=d.d AND g.sequence_no=2 LIMIT 1),'〜を皮切りに') AS p2,
    COALESCE((SELECT g.meaning_ko FROM grammar_pool g WHERE g.plan_id=p.id AND g.study_date=d.d AND g.sequence_no=2 LIMIT 1),'〜을 시작으로') AS m2
  ...
  SELECT b.*,
    ...
    CASE ((n+5)%8)
      ...
      ELSE '〜なくして' END AS d3,
    CASE (n%12)
      WHEN 0 THEN '制度を現場の実情に合わせて運用する場面'
      WHEN 1 THEN '予想外の結果でも冷静に判断する場面'
      WHEN 2 THEN '支援があって初めて成果が成立する場面'
      WHEN 3 THEN '厳しい条件でも行動を続ける場面'
      WHEN 4 THEN '責任ある立場として説明が求められる場面'
      WHEN 5 THEN '一度起きたことを簡単には元に戻せない場面'
      WHEN 6 THEN '想像することさえ難しい状況を述べる場面'
      WHEN 7 THEN '結果だけでなく過程も重視する場面'
      WHEN 8 THEN '専門家でも基本を守る必要がある場面'
...
SELECT plan_id,d,'grammar_question',1,'文法形式・'||d,
 json_object(
   'prompt','次の場面に最も合うN1文法表現を選びなさい：'||situation,
   'options',json_array(p1,d1,d2,d3),
   'answer',p1,
   'explanation','正解は「'||p1||'」。この日の学習上の意味は「'||m1||'」。意味と使用場面を結び付けて確認する。'
 )
FROM prepared
UNION ALL
SELECT plan_id,d,'grammar_question',2,'意味・用法・'||d,
 json_object(
   ...
   'answer',p2,
   'explanation','「'||p2||'」は「'||m2||'」の意味で用いる。接続と文脈も合わせて確認する。'
 )
FROM prepared
UNION ALL
...
-- One reading set per day. Theme and argument structure rotate independently.
WITH RECURSIVE days(d,n) AS (
  SELECT date('2026-09-07'),0
  UNION ALL
  SELECT date(d,'+1 day'),n+1 FROM days WHERE d<'2026-11-30'
  ...
  SELECT d.d,d.n,p.id AS plan_id,
    COALESCE(
      (SELECT group_concat(z.word,'・') FROM (
        ...
        WHERE c.plan_id=p.id AND c.introduced_on=d.d
  ...
  SELECT *,
    '「'||theme||'」について考えるとき、目に見える成果だけを基準に判断すると重要な点を見落としやすい。'||logic||
    ...
    ' 本日の語彙「'||key_words||'」も、こうした分析の文脈で意味を捉えると理解しやすい。良い判断とは情報を多く集めることそのものではなく、目的に照らして何を比較し、何を根拠として残すかを明確にすることだ。' AS passage
  FROM base
)
INSERT INTO japanese_jlpt_daily_contents(plan_id,study_date,content_type,sequence_no,title,payload_json)
 ...
 json_object(
   ...
   'questions',json_array(
     json_object(
       ...
       'explanation','本文では一つの数字を結論にせず、時点・立場・条件を比較し、変化の理由を確認する必要を述べている。'
     ),
     json_object(
       ...
       'explanation','一部の作業が簡単になっても、別の場所に負担が移れば全体の効率化とは限らないという例が示されている。'
     ),
     json_object(
...
-- Integrity check: all dates keep the full daily package.
...
WITH RECURSIVE days(d) AS (
  ...
  UNION ALL
  SELECT date(d,'+1 day') FROM days WHERE d<'2026-11-30'
), plans AS (
  SELECT id FROM japanese_jlpt_study_plans
  WHERE plan_code='N1_2027_JUL' AND is_active=1
)
...
SELECT CASE WHEN NOT EXISTS (
  ...
  WHERE (SELECT COUNT(*) FROM japanese_jlpt_daily_contents x WHERE x.plan_id=p.id AND x.study_date=days.d AND x.content_type='vocab_question')<>15
     ...
     OR COALESCE(json_array_length((SELECT json_extract(x.payload_json,'$.questions') FROM japanese_jlpt_daily_contents x WHERE x.plan_id=p.id AND x.study_date=days.d AND x.content_type='reading' LIMIT 1)),0)<>3
) THEN 1 ELSE 0 END;
