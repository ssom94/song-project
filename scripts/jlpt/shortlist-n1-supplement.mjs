...
const inputPath = path.join(root, 'data/jlpt/production/candidates/n1-supplement-review-queue.json');
const outputPath = path.join(root, 'data/jlpt/production/candidates/n1-seventh-batch-supplement-shortlist.json');
...
const holdout = ranked.slice(280).map((row) => ({ ...row, selection_status: 'held_out_from_batch_seven' }));
...
await fs.writeFile(outputPath, `${JSON.stringify({
  schemaVersion: 1,
  purpose: 'Editorial shortlist for the seventh 14-day N1-preparation batch supplement.',
  ...
  dateRange: { from: '2026-12-24', to: '2027-01-06', requiredWords: 280 },
