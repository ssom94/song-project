...
const isBatch3 = process.argv[2] === 'batch3';
const OUTPUT = path.join(PROD, 'candidates', isBatch3 ? 'n1-third-batch-proposals.json' : 'n1-second-batch-proposals.json');
...
if (isBatch3) {
	const second = JSON.parse(await fs.readFile(BATCH2, 'utf8'));
	for (const word of second.words) used.add(identity(word));
}
...
const words = candidates.slice(0, 280).map((word, index) => ({
	...word,
	sequence: index + (isBatch3 ? 561 : 281),
	planned_study_date: new Date(Date.UTC(2026, 9, (isBatch3 ? 29 : 15) + Math.floor(index / 20))).toISOString().slice(0, 10),
	review_status: 'proposed_for_admin_editorial_review',
	selection_reasons: [isBatch3 ? 'excluded first-and-second-batch identities' : 'excluded first-batch identities', 'verified local reading/meaning/example', word.priority_score >= 45 ? 'formal-or-abstract reading value' : 'secondary N1 editorial candidate'],
}));
if (words.length !== 280 || new Set(words.map(identity)).size !== 280) throw new Error(`Expected 280 unique ${isBatch3 ? 'third' : 'second'}-batch words; got ${words.length}`);

await fs.writeFile(OUTPUT, `${JSON.stringify({
	schemaVersion: 1,
	purpose: `${isBatch3 ? 'Third' : 'Second'} 14-day N1 editorial candidate batch. This is a project study priority, not an official per-word JLPT frequency ranking.`,
	dateRange: isBatch3 ? { from: '2026-10-29', to: '2026-11-11', dailyWords: 20 } : { from: '2026-10-15', to: '2026-10-28', dailyWords: 20 },
